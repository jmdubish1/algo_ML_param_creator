...
for lookback_range in lookback_ranges:
    for side in sides:
        ...
        dailydf, input_features = at.add_high_low_diff(dailydf, input_features, other_sec, sec_name)
        ...
        for d in range(0, len(end_dates)):
            ...
            # y_float_scaler = RobustScaler(quantile_range=(1, 99))
            y_float_scaler = MinMaxScaler(feature_range=(-1, 1))
            y_float_scaler.fit(combined_working[target_float_vars].values)

            combined_working = at.prep_ranks(combined_working)

            if subset_rank:
                combined_working = at.subset_by_rank(combined_working, rank_above, rank_below)

            rank_scaler = QuantileTransformer()
            rank_scaler.fit(combined_working[['Rank', 'Rank2']].values)

            x_float_scaler = RobustScaler(quantile_range=(1, 99))
            x_float_scaler.fit(daily_working[input_features].values)

            X_train, X_test, y_train, y_test = at.separate_train_test(combined_working, daily_working, input_features,
                                                                      test_size=test_size)

            X_train = at.vectorize_data(X_train, input_features, x_float_scaler, rank_scaler)
            X_test = at.vectorize_data(X_test, input_features, x_float_scaler, rank_scaler)
            y_train = at.vectorize_data(y_train, target_float_vars, y_float_scaler, rank_scaler)
            y_test = at.vectorize_data(y_test, target_float_vars, y_float_scaler, rank_scaler)

            full_size = int(y_train.shape[0]*data_per_epoch)
            y_test_size = max(int(len(y_test)*.25), 25)

            print(f'\n'
                  f'Side: {side}\n'
                  f'Start: {start_date}, End: {end_date}, Test_len: {X_train.shape[0]}\n'
                  f'Batch_s: {batch_s} \n'
                  f'Full_s: {full_size}\n'
                  f'Test_s: {y_test_size}')
            ...
            if os.path.exists(model_file):
                ...
                initializer3 = GlorotUniform()
                initializer4 = GlorotUniform()

                # print(f'X_shape : {X_train.shape}')
                # print(f'Y_shape : {y_train.shape}')
                input_layer = Input(shape=(X_train.shape[0], X_train.shape[1]))
                conv_1 = Conv1D(filters=6, kernel_size=3, activation='Relu', padding='same',
                                name='conv1')(input_layer)
                lstm_1 = LSTM(units=X_train.shape[1], activation='tanh', recurrent_activation='sigmoid',
                ...
                lstm_2 = LSTM(units=32, activation='tanh', recurrent_activation='sigmoid', return_sequences=False,
                              unroll=False, recurrent_dropout=0.0,
                              kernel_initializer=initializer2,
                              kernel_regularizer=l1(0.025),
                              bias_initializer=Constant(value=0.01),
                              name='lstm2')(drop_out1)
                drop_out2 = Dropout(0.1, name='drop2')(lstm_2)
                dense_1 = Dense(units=16, activation='tanh',
                                kernel_initializer=initializer3,
                                bias_initializer=Constant(value=0.01),
                                name='dense1')(drop_out2)
                float_output = Dense(units=9, activation='tanh',
                                     name='float_output')(dense_1)

                model = Model(inputs=input_layer,
                              outputs=[float_output])

                # Compile the model
                optimizer = Adam(learning_rate=.015)
            ...
            for epoch in range(epochs):
                ...
                if epoch % 2 == 0:
                    val_loss_avg = []
                    test_dataset = at.make_input_dataset(X_test, y_test, full_size=y_test_size, batch_s=batch_s)
                    total_loss = 0
                    num_batches = 0
            ...
            X_next_month_scaled = at.vectorize_data(fin_testDf, input_features, x_float_scaler, rank_scaler)
