...
class KMeansParams:
    ...
    def analyze_clusters(self,):
        ...
        for name, group in tempdf_clusterdf:
            temp_percentile = (
                group['avg_trade'].apply(lambda x: percentileofscore(group['avg_trade'], x) / 100))
            temp_percentile = temp_percentile.apply(lambda x: x * 2 - 1)
            group.loc[:, 'Rank'] = temp_percentile
        ...
        for name, group in tempdf_clusterdf:
            if (group['avg_trade'] < 0).all():
                ...
                group.loc[:, 'Rank'] = (
                    group['avg_trade'].apply(lambda x: percentileofscore(group['avg_trade'], x) / 100 - 1))
            else:
                temp_percentile = (
                    group['avg_trade'].apply(lambda x: percentileofscore(group['avg_trade'], x) / 100))
                temp_percentile = temp_percentile.apply(lambda x: x * 1.95 - 1)
                group.loc[:, 'Rank'] = temp_percentile

            sorted_groups.append(group)

        self.working_df = pd.concat([group for group in sorted_groups])
        """Below is for expirmentation"""
        # self.working_df.loc[:, 'Rank'] = np.where(self.working_df['Rank'] > 0.9, 1,
        #                                           np.where(self.working_df['Rank'] < -0.5, -1, 0))
        # self.working_df.loc[:, 'Rank'] = round(self.working_df.loc[:, 'Rank'], 6)
...
def result_dict_handler_dblc(predictions, y_scaler, target_float_vars, lookback_list, loss, lb_acc, end_date_test):
    ...
    print(result_dict['month_to_test'])
...
def complete_intraday_df_work(intraday_df, start_hour):
    ...
    intraAtr = round(intraday_df.groupby(intraday_df['Date'])['ATR_int'].mean(), 10)
    intraVol = round(intraday_df.groupby(intraday_df['Date'])['Vol_int'].mean(), 10)
...
def standardize_data(daily_df):
    ...
    for c in ['Vol', 'OpenInt', 'VolAvg', 'OI']:
        ...
        daily_df[c] = daily_df[c]/daily_df[c].shift(1)
...
def standardize_data_v2(daily_df):
    ...
    # Standardize by difference from last 20-day average
    for c in ['Vol', 'OpenInt']:
        daily_df[c] = daily_df[c].rolling(window=23, min_periods=1).mean()
        daily_df[c] = daily_df[c].pct_change() * 100

    return daily_df


def standardize_data_v3(data_df, daily_set_cols):
    ...
    for c in price_cols:
        ...
        data_df.loc[:, c] = temp_arr.astype(np.float32)
...
def create_best_df_dblc(combined_df, dailydf, lower_lookback_df, i,
                        equity_set_cols, start_date, end_date, months_to_train, month_size):

    train_df = create_random_df_percent_rank_dblc(combined_df, lower_lookback_df, rank=1)
...
def bootstrap_lookbacks(data_df, lookback_range):
    ...
    num_4_lookbacks = data_df.loc[data_df['lookback'] >= lookback_range[1]]

    bootstrapped = []
    for yearmonth in data_df['year_month'].unique():
        n_3 = len(num_3_lookbacks.loc[num_3_lookbacks['year_month'] == yearmonth])
        n_4_df = num_4_lookbacks.loc[num_4_lookbacks['year_month'] == yearmonth]
        n_4 = len(n_4_df)
        if n_4 < n_3:
            n_diff = int((n_3 - n_4)/6)
            sample_df = num_4_lookbacks.sample(n=n_diff, replace=True).reset_index(drop=True)
...
def vectorize_x(train_df, input_features, x_float_scaler):
    X = train_df[input_features].values
    X_scaled = x_float_scaler.transform(X)
    rank_reshape = train_df['Rank'].values.reshape(-1, 1)
    X_scaled = np.hstack((X_scaled, rank_reshape))
    rank_reshape = (1 / train_df['Rank']).values.reshape(-1, 1)
    X_scaled = np.hstack((X_scaled, rank_reshape))

    return X_scaled


def vectorize_x2(train_df, input_features):
    x_float_scaler = RobustScaler()
    X_scaled = x_float_scaler.fit_transform(train_df[input_features].values)
    rank_reshape = train_df['Rank'].values.reshape(-1, 1)
    X_scaled = np.hstack((X_scaled, rank_reshape))
    rank_reshape = (1 / train_df['Rank']).values.reshape(-1, 1)
    X_scaled = np.hstack((X_scaled, rank_reshape))
...
def vectorize_y_train(train_df, target_float_vars, y_float_scaler):
    y_float = train_df[target_float_vars].values
    y_float_scaled_train = y_float_scaler.transform(y_float)
    y_lookback = train_df['lookback'].values.reshape(-1, 1)
    y_takeprofit = train_df['takeProfit_oh'].values.reshape(-1, 1)
    y_dayslow = train_df['daySlowEma_oh'].values.reshape(-1, 1)

    return y_float_scaled_train, y_lookback, y_takeprofit, y_dayslow


def vectorize_y2(train_df, target_float_vars, y_float_scaler):
    y_float = train_df[target_float_vars].values
    y_float_scaled_train = y_float_scaler.transform(y_float)

    return y_float_scaled_train


def get_batch_replacement(array_scaled, n_lookback, n_steps):
    replacements = []
    for i in range(n_lookback + n_steps, len(array_scaled), n_steps):
        dat_arr = array_scaled[i-n_steps:i-1]
        replacements.append(random.choice(dat_arr))
...
def separate_train_test(X, y, n_steps, n_lookback, test_size, input_features):
    X.reset_index(inplace=True, drop=True)
    range_mask = range(n_lookback, len(X), n_steps)
    test_int = int(len(range_mask) * test_size)
    test_idx = np.random.choice(range(0, len(range_mask)), size=test_int, replace=False)
    test_idx.sort()
    train_idx = np.array([int(i) for i in range(0, len(range_mask)) if i not in test_idx])
    X = X[input_features + ['Rank']]

    x_output = []
    reshaped_x = [X.iloc[(i-n_lookback):i, :].copy() for i in range_mask]
    for x in reshaped_x:
        output = vectorize_x2(x, input_features)
        x_output.append(output)

    X_train = []
    X_test = []
    for i in range(len(range_mask)):
        if i in test_idx:
            X_test.append(x_output[i])
        else:
            X_train.append(x_output[i])

    y_reshaped = reshape_X_n_steps(y, n_lookback, n_steps)
    y_train = y_reshaped[train_idx]
    y_test = y_reshaped[test_idx]

    return X_train, X_test, y_train, y_test


# def make_input_dataset(X, y_float, y_lookback, y_takeprofit, y_dayslow, modify=False):
#     y_arr = []
#     for i in range(0, len(X)):
#         X[i][:, -2:] = random.choice(X[i][-22:, -2:])
#         y_f = random.choice(y_float[i][-22:])
#         print(y_f)
#         y_l = random.choice(y_lookback[i][-22:])
#         print(y_l)
#         y_t = random.choice(y_takeprofit[i][-22:])
#         print(y_t)
#         y_d = random.choice(y_dayslow[i][-22:])
#         print(y_d)
#         temp_arr = np.array([y_f, y_l, y_t, y_d])
#         y_arr.append(temp_arr)
#
#     print(y_arr)
...
#         y_arr = tf.convert_to_tensor(y_arr)
#         return X, y_arr
#     else:
#         dataset = tf.convert_to_tensor((X, y_arr))
#         return dataset


def make_input_dataset(X, y_float, batch_s):
    y_arr = []
    for i in range(0, len(X)):
        randint = random.randint(1, 23)
        X[i][:, -2:] = random.choice(X[i][-randint, -2:])
        y_arr.append(random.choice(y_float[i][-randint]))
    y_arr = np.stack(y_arr, axis=0)

    y_arr = tf.expand_dims(y_arr, axis=-1)
    y_arr = tf.expand_dims(y_arr, axis=-1)
    dataset = tf.data.Dataset.from_tensor_slices((X, y_arr))
